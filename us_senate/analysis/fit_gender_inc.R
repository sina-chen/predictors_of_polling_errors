########################################################################################
# Senate poll accuracy, gender and incumbency
# Author: Sina Chen
#
############################################################################################
...
vote_sy <- data_complete %>%
  group_by(state_year, election_year, state, rep_result2, gender_rep_dummy, gender_dem_dummy, inc_dummy) %>%
  summarise()
...
resStan_gender <- stan(file = "stan_ml/ml_senate_gender_inc.stan", data = stan_dat,
                chains = 4, iter = 5000,
                control = list(adapt_delta = 0.95)
)    

launch_shinystan(res_stan_gender$stand_fit)

saveRDS(list(stand_fit = resStan_gender), 'res_stan_gender.RDS')
