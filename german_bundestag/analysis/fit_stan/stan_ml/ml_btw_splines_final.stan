...
parameters {
  ...
  row_vector[n_B] beta_sc[KR-R]; // scale for non-centerred parametrisation
  ...
  vector[KR] phi_sc; // scale for non-centerred parametrisation
  real<lower=0>  sig_phi; // sd for non-centerred parametrisation
...
model {
  ...
  vector[N] sigma; // standard deviation

  // hyper-priors for non-centered parametrisation
